from pydantic_settings import BaseSettings
from typing import List
import os

class Settings(BaseSettings):
    # Database
    DATABASE_URL: str = "sqlite:///sullivan.db"
    
    # GMGN API
    GMGN_API_KEY: str
    GMGN_ED25519_PRIVATE_KEY: str  # Hex-encoded Ed25519 private key
    GMGN_RATE_LIMIT_PER_MINUTE: int = 100
    
    # Claude API
    ANTHROPIC_API_KEY: str
    MAX_CLAUDE_CALLS_PER_HOUR: int = 100
    CLAUDE_CACHE_TTL_SECONDS: int = 1800  # 30 minutes
    
    # Telegram
    TELEGRAM_BOT_TOKEN: str
    TELEGRAM_CHAT_ID: str
    
    # Redis
    REDIS_URL: str = "redis://localhost:6379"
    
    # Trading
    SUPPORTED_CHAINS: List[str] = ["sol", "base", "eth", "bsc"]
    MIN_SIGNAL_SCORE: int = 65
    MAX_OPEN_POSITIONS: int = 10
    
    # Risk Management
    TRAILING_STOP_PERCENT: float = 15.0  # 15% trailing stop
    TAKE_PROFIT_PERCENT: float = 100.0   # 100% take profit
    DAILY_LOSS_LIMIT_PERCENT: float = 10.0  # 10% daily loss limit
    WEEKLY_LOSS_LIMIT_PERCENT: float = 20.0  # 20% weekly loss limit
    
    # Position Sizing
    DEFAULT_POSITION_SIZE_USD: float = 100.0
    MAX_POSITION_SIZE_USD: float = 500.0
    
    # Authentication
    JWT_SECRET_KEY: str = "your-secret-key-change-in-production"
    JWT_ALGORITHM: str = "HS256"
    JWT_EXPIRATION_HOURS: int = 24

    class Config:
        env_file = ".env"

settings = Settings()
